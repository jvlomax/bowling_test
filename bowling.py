...
class Game:
    ...
    def total_score(self):
        ...
        for idx, frame in enumerate(self._frames):
            ...
            if frame.is_strike:
                ...
                # We also want to add the next two rolls if the frame has been rolled
            ...
            # Process rule for a spare
                ...
                score += sum(frame.rolls)
                # If the next frame exists, we get the first roll from it and it it to the score for this frame.
                if len(self._frames) > idx + 1:
    ...
    def add_frame(self, frame):
        ...
        # The last roll is the fill roll
        if frame.rolls[-1]:
...
class Frame:
    ...
    features.
    """
    def __init__(self, roll1, roll2=0, fill_roll=None):
        """
        ...
        :raises AttirbuteError: raises and exception if the number of pins the total number of pins is over 10
                                or is negative
        """
        # We have to do a little extra logic on the extra ball. If it's defined, we check it's value and if it's null,
        # we just assign it to be zero.
        if not fill_roll:
            fill_roll = 0
